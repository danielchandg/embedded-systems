...
Code code_init(void){
	Code c;
	c.top = 0;
	for(int i=0; i<MAX_CODE_SIZE; i++) c.bits[i] = 0;
	return c;
}
...
bool code_push_bit(Code *c, uint8_t bit){
	if(c->top == ALPHABET) return false;
	if(bit==1) code_set_bit(c, c->top);
	else code_clr_bit(c, c->top);
	c->top++;
	return true;
}
bool code_pop_bit(Code *c, uint8_t *bit){
	if(c->top == 0) return false;
...
void code_print(Code *c){
	...
	for(uint32_t i=0; i<c->top; i++){
		if(code_get_bit(c, i)) printf("1");
		else printf("0");
		if(i != c->top - 1 && i%8 == 7) printf(" ");
	}
	printf("\n");
}
