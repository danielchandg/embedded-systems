...
int main(int argc, char **argv){
	...
	while((opt = getopt(argc, argv, OPTIONS)) != -1) {
		switch (opt) {
			...
			case 't':
				...
				if(errno == EINVAL) HashTableSize = (1 << 16);
				break;
			case 'f':
				BloomFilterSize = strtoul(optarg, NULL, 10);
				if(errno == EINVAL) BloomFilterSize = (1 << 20);
				break;
			case 's':
				stats = true;
				break;
	...
	char *badWord = (char *)calloc(1024, sizeof(char));
	char *goodWord = (char *)calloc(1024, sizeof(char));
	...
	if(stats){
		...
		if(bad_list && old_list) fprintf(stdout, "%s", mixspeak_message);
		else if(bad_list)
