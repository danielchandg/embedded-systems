...
int main(int argc, char **argv){
	...
	while((opt = getopt(argc, argv, OPTIONS)) != -1){
		...
		case 'a':
			...
			break;
		case 'e':
			opts = insert_set(0, opts);
			break;
		case 's':
			opts = insert_set(1, opts);
			break;
		case 'i':
			opts = insert_set(2, opts);
			break;
		case 'q':
			opts = insert_set(3, opts);
			break;
		case 'r':
	...
	if(member_set(0, opts)){
		...
		printf("Heap Sort, %" PRIu32 " elements, %" PRIu64 " moves, %" PRIu64 " compares", elements, heap_stats.moves, heap_stats.compares);
		print_arr(A, print);		
		for(uint32_t i=0; i<elements; i++) A[i] = arr[i];
	}
	if(member_set(1, opts)){
                reset(&shell_stats);
                shell_sort(&shell_stats, A, elements);
                printf("Shell Sort, %" PRIu32 " elements, %" PRIu64 " moves, %" PRIu64 " compares", elements, shell_stats.moves, shell_stats.compares);
                print_arr(A, print);
                for(uint32_t i=0; i<elements; i++) A[i] = arr[i];
        }
	...
	if(member_set(2, opts)){
		...
		printf("Insertion Sort, %" PRIu32 " elements, %" PRIu64 " moves, %" PRIu64 " compares", elements, insert_stats.moves, insert_stats.compares);
		print_arr(A, print);
                for(uint32_t i=0; i<elements; i++) A[i] = arr[i];
	}
	if(member_set(3, opts)){
                ...
                printf("Quick Sort, %" PRIu32 " elements, %" PRIu64 " moves, %" PRIu64 " compares", elements, quick_stats.moves, quick_stats.compares);
                print_arr(A, print);
                for(uint32_t i=0; i<elements; i++) A[i] = arr[i];
        }
