...
int main(int argc, char** argv){
	...
	if(args[0]){
		...
		if(args[6]) printf("e() terms = %d\n", e_terms());
	}
	if(args[3]){
		pi = pi_euler();
		printf("pi_euler() = %16.15lf, M_PI = %16.15lf, diff = %16.15lf\n", pi, M_PI, absolute(pi - M_PI));
		if(args[6]) printf("pi_euler() terms = %d\n", pi_euler_terms());
	}
	if(args[1]){
		pi = pi_bbp();
		printf("pi_bbp() = %16.15lf, M_PI = %16.15lf, diff = %16.15lf\n", pi, M_PI, absolute(pi - M_PI));
		if(args[6]) printf("pi_bbp() terms = %d\n", pi_bbp_terms());
	}
	if(args[2]){
		pi = pi_madhava();
                printf("pi_madhava() = %16.15lf, M_PI = %16.15lf, diff = %16.15lf\n", pi, M_PI, absolute(pi - M_PI));
                if(args[6]) printf("pi_madhava() terms = %d\n", pi_madhava_terms());
	}
	if(args[4]){
		pi = pi_viete();
                printf("pi_viete() = %16.15lf, M_PI = %16.15lf, diff = %16.15lf\n", pi, M_PI, absolute(pi - M_PI));
                if(args[6]) printf("pi_viete() terms = %d\n", pi_viete_factors());
	}
	if(args[5]){
		for (double i = 0; i<=10; i+=0.1){
			double sq1 = sqrt_newton(i), sq2 = sqrt(i);
			printf("sqrt_newton(%7.6lf) = %16.15lf, sqrt(%7.6lf) = %16.15lf, diff = %16.15lf\n", i, sq1, i, sq2, absolute(sq1-sq2));
			if(args[6]) printf("sqrt_newton() terms = %d\n", sqrt_newton_iters());
		}
	}
	return 0;
}
